...
void main(uint group_Index : SV_GROUPINDEX, uint2 group_ID : SV_GROUPID, uint2 thread_ID : SV_DISPATCHTHREADID, uint2 group_thread_ID : SV_GroupThreadID)
{
    ...
    //output_average_motion_vector[int2(0)] = float2(0.f);
    ...
    if (camera_moved) {
        ...
        int2 screen_coord;
        screen_coord.x = ((reprojected_position_clip.x + 1.f) / 2.f) * (frame_width);
        screen_coord.y = ((reprojected_position_clip.y - 1.f) / -2.f) * (frame_height);

        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        float2 average_diff = ((float2) screen_coord - (float2) thread_ID) / (float) (frame_width * frame_height);

        //garantee an atomic access!
        output_average_motion_vector[int2(0)] += average_diff; //thread_ID / float2(frame_width, frame_height);
